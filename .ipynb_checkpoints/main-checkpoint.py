...
def main():
    ...
    for exp in range(start_exp, args.num_exp):
        ...
        if args.save_path != "":
            ...
            # create the logger
            new_folder_name = os.path.join(args.save_path, checkpoint_name)
            if not os.path.exists(new_folder_name):
                os.makedirs(new_folder_name)
                
            fname = os.path.join(new_folder_name, 'deepcore_exp.log')
            logging.basicConfig(level=logging.INFO, filename=fname, filemode='w')
        else:
            logging.basicConfig(level=logging.INFO, filename='deepcore_exp.log', filemode='w')
            checkpoint_name = ''
        ...
        logger.info(f"""
            dataset: {args.dataset}, model: {args.model}, selection: {args.selection},
            num_ex: {args.num_exp}, epochs: {args.epochs}, fraction: {args.fraction}, seed: {args.seed},
              lr: {args.lr}, save_path: {args.save_path}, resume: {args.resume}, device: {args.device},
            """)
        ...
        for model in models:
            ...
            # deal with checkpoint later
            new_record = NewRecord(save_path = args.save_path, checkpoint_name=checkpoint_name)
            ...
            for epoch in range(start_epoch, args.epochs):
                ...
                train(train_loader, network, criterion, optimizer, scheduler, epoch, args, rec, 
                      new_record, if_weighted=if_weighted)

                # evaluate on validation set
                if args.test_interval > 0 and (epoch + 1) % args.test_interval == 0:
                    prec1 = test(test_loader, network, criterion, epoch, args, rec, new_record)
            ...
            logger.info(f"Experiment {exp} ended at: {experiment_stop_time}")
            logger.info(f"Experiment duration: {exp_duration}")
