...
def train(train_loader, network, criterion, optimizer, scheduler, epoch, args, rec, new_record, if_weighted: bool = False):
    ...
    new_record.add_training_record(step=epoch, loss=losses.avg, acc=top1.avg, lr=optimizer.state_dict()['param_groups'][0]['lr'])
    


def test(test_loader, network, criterion, epoch, args, rec, new_record):
    batch_time = AverageMeter('Time', ':6.3f')
    losses = AverageMeter('Loss', ':.4e')
    top1 = AverageMeter('Acc@1', ':6.2f')
    ...
    new_record.add_test_record(step=epoch, loss=losses.avg, acc=top1.avg)
    return top1.avg
...
class NewRecord:
    def __init__(self, save_path,  checkpoint_name):
        self.save_path = save_path
        self.checkpoint_name = checkpoint_name
        self.training_records = []
        self.test_records = []
    
    def add_training_record(self, step, loss, acc, lr):
        record = ResultRecord(step=step, loss=loss, acc=acc, lr=lr)
        self.training_records.append(record)
    
    def add_test_record(self, step, loss, acc, lr=0):
        record = ResultRecord(step=step, loss=loss, acc=acc, lr=0)
    ...
    def compile_and_save(self):
        ...
        training_df = pd.DataFrame(
            [asdict(x) for x in self.training_records]
        )
        
        test_df = pd.DataFrame(
            [asdict(x) for x in self.test_records]
        )
        
        training_fname = os.path.join(
            self.save_path,self.checkpoint_name,
            'training_accuracies.csv'
        )
...
def get_duration(start_time, stop_time):
    duration_secs = stop_time - start_time
    if duration_secs > 3600:
        num_hours = int(np.floor(duration_secs/3600))
        num_mins = int(np.floor(duration_secs/60)) - (num_hours * 60)
        num_secs = int(duration_secs % 60)
        
        return f'{num_hours} hours, {num_mins} mins, {num_secs} seconds'
    else:
        num_mins = int(np.floor(duration_secs/60))
        num_secs = int(duration_secs % 60)
        return f'{num_mins} mins, {num_secs} seconds'
